...
from database import registerUserBot # para registrar al usuario desde el bot
from database import getUsers # para obtener los usuarios desde el bot
from raspberry.llamada_policia import llamarPolicia # para llamar a la policia
import raspberry.funciones as rp # para usar las funciones de la raspberry
from forms.dashboard.form_master_design import Panel # para el sistema de login
...
def callback_query(call):
    if call.data == 'activar_alarma':
        ...
        rp.monitorearCamara()
        time.sleep(3)
        bot.edit_message_text('<a href="http://10.206.23.220:81/stream">Haga click aquí para monitorear la cámara</a>', call.message.chat.id, respuesta_camara.message_id, parse_mode="html")
